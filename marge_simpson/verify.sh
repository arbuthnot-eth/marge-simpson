...
PROFILE="${1:-fast}"  # fast | full

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
MS_DIR="$ROOT_DIR/marge_simpson"
...
main() {
  say "[marge_simpson] verify profile=$PROFILE"
  say "[marge_simpson] repo_root=$ROOT_DIR"
  say "[marge_simpson] log=$LOG_FILE"
