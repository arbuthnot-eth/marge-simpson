<#
marge_simpson/verify.ps1

Runs repo verification commands (tests/lint/build) and writes a timestamped log to
`marge_simpson/verify_logs/`.
...
Behavior:
  - If `marge_simpson/verify.config.json` exists and has non-empty commands for the profile, those are run in order.
  - Otherwise, the script attempts to autodetect common stacks.
...
$MsDir = Resolve-Path $PSScriptRoot
...
Write-Log ("[marge_simpson] verify profile={0}" -f $Profile)
Write-Log ("[marge_simpson] repo_root={0}" -f $RootDir)
Write-Log ("[marge_simpson] log={0}" -f $LogFile)
...
try {
  ...
  Write-Log ""
  Write-Log ("PASS: verify ({0})" -f $Profile)
  exit 0
}
catch {
  Write-Log ""
  Write-Log ("FAIL: verify ({0})" -f $Profile)
  Write-Log $_
  exit 1
}
